...
describe("limitCapabilityOutput", () => {
  it("limits line and byte output with a deterministic notice", () => {
    for (const text of ["x\n".repeat(2_100), "é".repeat(60_000)]) {
      ...
      expect(limited.content[0]?.text).toContain(
        "[Capability output truncated]",
      );
      expect(
        Buffer.byteLength(limited.content[0]?.text ?? ""),
      ).toBeLessThanOrEqual(50 * 1024);
      expect(
        limited.content[0]?.text.split("\n").length ?? 0,
  ...
  it("prioritizes MCP text before adding JSON details to the shared byte budget", () => {
    ...
    const limited = limitCapabilityOutput({
      content: [{ type: "text", text }],
      details: { value: "d".repeat(30 * 1024) },
    });
  ...
  it("omits JSON details that exceed the output boundary or cannot be serialized", () => {
    ...
    expect(
      limitCapabilityOutput({
        content: [{ type: "text", text: "ok" }],
        details: { value: "bounded" },
      }),
    ).toHaveProperty("details", { value: "bounded" });
  });
});
