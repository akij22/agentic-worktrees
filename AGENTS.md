...
* Use the graph generated by `graphify` and its generated Markdown report to explore the files and learn the project's context before making changes.
* Consult `graphify-out/graph.json` for relationships between files and concepts, and `graphify-out/GRAPH_REPORT.md` for the generated project overview and navigation hubs.
