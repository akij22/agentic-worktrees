...
const api: Api = {
	github: {
		auth: {
			...
			completeLogin: () =>
				ipcRenderer
					.invoke(IPC_CHANNELS.GITHUB_AUTH_COMPLETE)
					.then((value) => githubAuthStatusSchema.parse(value)),
			...
			openDeviceVerification: () =>
				ipcRenderer.invoke(IPC_CHANNELS.GITHUB_AUTH_OPEN_DEVICE_VERIFICATION),
			openInstallation: () =>
		...
		files: {
			...
			search: async (request) =>
				workspaceFileSearchResponseSchema.parse(
					await ipcRenderer.invoke(IPC_CHANNELS.WORKSPACE_FILE_SEARCH, request),
				),
		},
		terminal: {
		...
		git: {
			...
			openPullRequest: async (request) =>
				workspacePullRequestResultSchema.parse(
					await ipcRenderer.invoke(IPC_CHANNELS.WORKSPACE_GIT_OPEN_PR, request),
				),
		},
	},
	intelligence: {
	...
	codingAgent: {
		...
		onEvent: (listener) => {
			...
			return () =>
				ipcRenderer.removeListener(IPC_CHANNELS.CODING_AGENT_EVENT, handler);
		},
	},
};
