...
export const useConflictPreparation = (
	...
	useEffect(() => {
		...
		void Promise.all([
			...
			window.api.intelligence.listResolutionSessions({
				repositoryId,
				overlapId,
			}),
		])
			.then(([values, sessions]) => {
				...
				setTargetBranch((current) => {
					if (current && values.some(({ name }) => name === current))
						return current;
					if (
						defaultBranch &&
						values.some(({ name }) => name === defaultBranch)
					)
	...
	useEffect(
		() =>
			window.api.intelligence.onResolutionSessionChanged((event) => {
				if (event.repositoryId !== repositoryId) return;
				void reloadSessions().catch((cause: unknown) =>
	...
	const prepare = useCallback(async () => {
		...
		try {
			setSession(
				await window.api.intelligence.prepareConflict({
					overlapId,
					targetBranch,
				}),
			);
