...
const session = (
	...
	currentStage:
		state === "requested" ? "Requested" : "Semantic review required",
	integrationBranch:
		state === "review_required" ? "agentic/integration/session-1" : null,
	integrationPath:
		state === "review_required" ? "/integration/session-1" : null,
...
describe("useConflictPreparation", () => {
	...
	it("reloads the selected overlap after a persisted session event", async () => {
		...
		Object.defineProperty(window, "api", {
			...
			value: {
				intelligence: {
					...
					onResolutionSessionChanged: vi.fn((value) => {
						listener = value;
						return () => undefined;
					}),
				},
		...
		await waitFor(() =>
			expect(result.current.session?.state).toBe("requested"),
		);
		...
		await waitFor(() =>
			expect(result.current.session?.state).toBe("review_required"),
		);
	});
});
