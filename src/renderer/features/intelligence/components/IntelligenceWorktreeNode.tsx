...
export const IntelligenceWorktreeNode = ({ worktree, risk, onOpenChat }: Props) => (
  <article
  ...
  >
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0">
        ...
        <h3 className="mt-2 truncate text-sm font-semibold" title={worktree.task}>{worktree.task}</h3>
    ...
    <div className="mt-4 grid grid-cols-3 gap-2 border-y border-border/70 py-3 text-center font-mono text-[10px]">
      <div><strong className="block text-sm text-foreground">{worktree.changedFileCount}</strong><span className="text-muted-foreground">files</span></div>
      <div><strong className="block text-sm text-emerald-400">+{worktree.additions}</strong><span className="text-muted-foreground">added</span></div>
    ...
    <div className="mt-3 flex items-center justify-between gap-2">
      ...
      <Button
        ...
        onClick={() => worktree.runId && onOpenChat(worktree.worktreeId, worktree.runId)}
