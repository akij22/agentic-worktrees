...
export const WorktreeOverlapMap = ({
	snapshot,
	onReview,
	onCompare,
	onOpenChat,
	...
	const visible = snapshot.worktrees.slice(
		safePage * PAGE_SIZE,
		(safePage + 1) * PAGE_SIZE,
	);
	const visibleIds = useMemo(
		() => new Set(visible.map(({ worktreeId }) => worktreeId)),
		[visible],
	);
	const relationships = snapshot.overlaps.filter(
		({ leftWorktreeId, rightWorktreeId }) =>
			visibleIds.has(leftWorktreeId) && visibleIds.has(rightWorktreeId),
	);
	const riskFor = (worktreeId: string) =>
		snapshot.overlaps
			.filter(
				({ leftWorktreeId, rightWorktreeId }) =>
					leftWorktreeId === worktreeId || rightWorktreeId === worktreeId,
			)
			.reduce<"low" | "medium" | "high">(
				(current, overlap) =>
					riskRank[overlap.risk] > riskRank[current] ? overlap.risk : current,
				"low",
			);
	...
	return (
		<section
			...
			aria-labelledby="overlap-map-heading"
		>
			<div className="flex items-center justify-between gap-4 border-b border-border px-4 py-3">
				<div>
					<h2
						id="overlap-map-heading"
						className="flex items-center gap-2 text-sm font-semibold"
					>
						<GitMerge className="size-4 text-muted-foreground" /> Worktree
						overlap map
					</h2>
			...
			<div className="relative p-4">
				...
				{relationships.length > 0 ? (
					<div className="pointer-events-none absolute left-1/2 top-1/2 z-10 hidden -translate-x-1/2 -translate-y-1/2 flex-col items-center rounded-lg border border-border bg-background/95 px-3 py-2 shadow-lg lg:flex">
						...
						<span className="mt-1 font-mono text-[8px] font-bold uppercase tracking-widest text-muted-foreground">
							Analysis engine
						</span>
					</div>
				) : null}
				<ul className="sr-only">
					{relationships.map((overlap) => (
						<li key={overlap.id}>
							{overlap.risk} risk: {overlap.summary}
						</li>
					))}
				</ul>
				<div className="relative grid gap-4 lg:grid-cols-2 lg:gap-x-28">
					{visible.map((worktree) => (
						<IntelligenceWorktreeNode
							...
							worktree={worktree}
							risk={riskFor(worktree.worktreeId)}
				...
				{relationships.length > 0 ? (
					<div
						className="relative mt-4 flex flex-wrap gap-2 border-t border-border pt-3"
						aria-label="Visible relationships"
					>
						{relationships.map((overlap) => (
							<div
								key={overlap.id}
								className="flex items-center rounded-md border border-border bg-background/80"
							>
								<button
									type="button"
									className="flex items-center gap-2 px-2.5 py-1.5 text-left text-[10px] hover:bg-accent"
								...
								<button
									...
									aria-label={`Compare ${overlap.summary}`}
