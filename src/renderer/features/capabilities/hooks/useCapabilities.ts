...
export function useCapabilities(runId?: string) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (selectedId.current) setDetail(await window.api.capabilities.get({ capabilityId: selectedId.current, ...(runId ? { runId } : {}) }));
      setError(undefined);
    } catch { if (mounted.current) setError("Could not load capabilities. Please try again."); }
    finally { refreshPending.current = false; if (mounted.current) setLoading(false); }
  }, [runId]);

  useEffect(() => {
    mounted.current = true; setLoading(true); setDetail(undefined); selectedId.current = undefined; void refresh();
  ...
  const select = useCallback(async (capabilityId: string) => {
    ...
    catch { if (mounted.current) setError("Could not load capability details."); }
  }, [runId]);
