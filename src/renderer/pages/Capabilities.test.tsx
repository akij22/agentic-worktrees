...
function installApi(state: "available" | "ready" = "available") {
  ...
  Object.defineProperty(window, "api", {
    ...
    value: {
      capabilities: {
        list: vi.fn().mockResolvedValue([contextualSummary]),
        get: vi.fn().mockResolvedValue(contextualDetail),
        configure: vi.fn(),
        activate,
        deactivate: vi.fn(),
...
describe("Capabilities library", () => {
  it("searches and opens reviewed capability details with compatibility", async () => {
    ...
    expect(await screen.findByText("Permission ledger")).toBeTruthy();
    expect(screen.getByText("Compatibility & source")).toBeTruthy();
    expect(screen.getAllByText("Codex")).toHaveLength(2);
    expect(screen.getAllByText("Supported")).toHaveLength(2);
    await userEvent.type(
      screen.getByLabelText("Search capabilities"),
      "missing",
    );
    expect(screen.getByText("No capabilities match.")).toBeTruthy();
  ...
  it("adds a ready capability to the originating chat context", async () => {
    ...
    render(
      <MemoryRouter
        initialEntries={[
          { pathname: "/capabilities", state: { runId: "run-1" } },
        ]}
      >
        <Capabilities />
      </MemoryRouter>,
