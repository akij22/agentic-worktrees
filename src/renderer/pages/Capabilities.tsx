...
export const Capabilities = () => {
  ...
  const library = useCapabilities(runId); const [query, setQuery] = useState(""); const [state, setState] = useState("all"); const [setupOpen, setSetupOpen] = useState(false);
  const filtered = useMemo(() => library.capabilities.filter((capability) => (`${capability.name} ${capability.description}`.toLowerCase().includes(query.toLowerCase()) && (state === "all" || capability.state === state))), [library.capabilities, query, state]);
  ...
  return <div className="grid h-full min-h-[32rem] overflow-hidden rounded-xl border border-border/70 bg-card/25 lg:grid-cols-[20rem_minmax(0,1fr)]">
    <aside className="flex min-h-0 flex-col border-b border-border/70 lg:border-b-0 lg:border-r"><div className="border-b border-border/70 p-4"><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-primary">Curated library</p><p className="mt-1 text-xs text-muted-foreground">Reviewed local tools for coding-agent chats.</p><div className="relative mt-4"><Search className="absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input aria-label="Search capabilities" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search" className="pl-8" /></div><Select aria-label="Filter capability state" className="mt-2" value={state} onChange={(event) => setState(event.target.value)}><option value="all">All states</option><option value="available">Available</option><option value="needs_setup">Needs setup</option><option value="ready">Ready</option><option value="unavailable">Unavailable</option></Select></div>
    ...
    <main className="min-h-0">{library.detail ? <CapabilityDetail capability={library.detail} onConfigure={() => setSetupOpen(true)} /> : <div className="grid h-full place-items-center text-sm text-muted-foreground">Select a capability</div>}</main>
