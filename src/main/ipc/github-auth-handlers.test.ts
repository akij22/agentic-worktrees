...
describe("GitHub authentication IPC handlers", () => {
	...
	it.each([
		...
		[IPC_CHANNELS.GITHUB_AUTH_REFRESH_INSTALLATIONS, "refreshInstallations"],
		...
		const publicResult =
			channel === IPC_CHANNELS.GITHUB_AUTH_START
				? {
						userCode: "ABCD-EFGH",
	...
	it.each(["signed_out", "installation_required"])(
		...
		async (state) => {
			...
			expect(mocks.assertAuthenticated).toHaveBeenCalledTimes(
				applicationChannels.length,
			);
		},
	);
	...
	it("delegates an application operation after authenticated assertion", async () => {
		...
		await expect(
			invoke(IPC_CHANNELS.GITHUB_LIST_REMOTE_REPOS),
		).resolves.toEqual([{ githubRepoId: 1 }]);
	...
	it("validates and delegates workspace file search", async () => {
		...
		expect(mocks.searchFiles).toHaveBeenCalledWith("worktree-1", "app", 20);
	});
	...
	it("validates and delegates an intelligence refresh", async () => {
		...
		await expect(
			invoke(IPC_CHANNELS.INTELLIGENCE_REFRESH, {
				repositoryId: "repository-1",
			}),
	...
	it("validates and delegates conflict preparation", async () => {
		...
		await expect(
			invoke(IPC_CHANNELS.INTELLIGENCE_CONFLICT_PREPARE, {
				overlapId: "overlap-1",
				targetBranch: "main",
			}),
	...
	it("broadcasts parsed terminal events to renderer windows", () => {
		...
		expect(send).toHaveBeenCalledWith(IPC_CHANNELS.WORKSPACE_TERMINAL_EVENT, {
			type: "data",
			worktreeId: "worktree-1",
			terminalId: "terminal-1",
			data: "ready",
	...
	it("broadcasts only parsed public authentication status changes", () => {
		...
		expect(send).toHaveBeenCalledWith(IPC_CHANNELS.GITHUB_AUTH_STATUS_CHANGED, {
			state: "signed_out",
			profile: null,
			installationCount: 0,
			persistent: true,
