...
beforeEach(() => {
	...
	sqlite.prepare(`INSERT INTO repositories (
		...
		.run("repository-1", 1, "local", "repo", "local/repo", "main", 0, 0, "", null, "", "/repo", "ready", now, now);
});
...
describe("conflict resolution repository", () => {
	it("round trips normalized participants, files, and operations", () => {
		const repository = createConflictResolutionRepository(drizzle(sqlite, { schema }));
		repository.createSession(session());

		expect(repository.getSession("session-1")).toEqual(session());
		expect(repository.listSessions("repository-1")).toEqual([session()]);
		expect(repository.findActive("repository-1", "overlap-1", "main")?.id).toBe("session-1");
	});
	...
	it("rolls back a hierarchy replacement when a child insert fails", () => {
		...
		invalid.operations = [invalid.operations[0], { ...invalid.operations[0], id: "operation-2" }];

		expect(() => repository.saveSession(invalid)).toThrow();
		expect(repository.getSession("session-1")?.currentStage).toBe("Simulating merge");
	});
});
