...
const adapter = (): IntegrationGitAdapter => ({
	...
	createSyntheticSnapshot: vi
		.fn()
		.mockResolvedValueOnce(snapshot("left"))
		.mockResolvedValueOnce(snapshot("right")),
	createIntegrationWorktree: vi
		.fn()
		.mockResolvedValue({
			path: "/integration/session",
			branch: "agentic/integration/session",
		}),
	mergeSynthetic: vi
		.fn()
		.mockResolvedValueOnce({
			kind: "clean",
			files: [],
			mergeCommitSha: "left-merge",
		})
		.mockResolvedValueOnce({
			kind: "clean",
			files: [],
			mergeCommitSha: "right-merge",
		}),
...
describe("IntegrationWorktreeService", () => {
	it("removes disposable Git state after a safe merge", async () => {
		const git = adapter();
		const result = await createIntegrationWorktreeService({ git }).prepare(
			input(),
		);
	...
	it("rejects participants outside the repository before Git operations", async () => {
		...
		const invalid = {
			...
			participants: [
				...
				{ ...valid.participants[1], repositoryId: "other" },
			] as const,
		};

		await expect(
			createIntegrationWorktreeService({ git }).prepare(invalid),
	...
	it("cleans partial resources after a failed merge", async () => {
		...
		vi.mocked(git.mergeSynthetic)
			.mockReset()
			.mockRejectedValue(new Error("merge crashed"));

		await expect(
			createIntegrationWorktreeService({ git }).prepare(input()),
		).rejects.toThrow("merge crashed");
		expect(git.removeIntegrationWorktree).toHaveBeenCalledOnce();
		expect(git.deleteSyntheticRef).toHaveBeenCalledTimes(2);
	});
});
