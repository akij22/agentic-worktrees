...
describe("GitProcess", () => {
	it("passes Git arguments without a shell", async () => {
		...
		await expect(process.run({ cwd: "/repo", args: ["status", "--porcelain=v2"] }))
			.resolves.toEqual({ stdout: "ok\n", stderr: "" });
		expect(runner).toHaveBeenCalledWith("git", ["status", "--porcelain=v2"], expect.objectContaining({ cwd: "/repo" }));
	});
	...
	it("rejects unsafe ref names before invoking Git", async () => {
		...
		await expect(process.assertSafeRef("--upload-pack=evil")).rejects.toThrow(/Unsafe Git ref/);
		expect(runner).not.toHaveBeenCalled();
	});

	it("bounds output and preserves non-zero command context", async () => {
		const runner = vi.fn().mockRejectedValue(Object.assign(new Error("failed"), {
			stdout: "x".repeat(100),
		...
		const error = await process.run({ cwd: "/repo", args: ["merge", "ref"] }).catch((cause: unknown) => cause);
