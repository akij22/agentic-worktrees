...
const safeSessionId = (value: string): string => {
	if (!/^[A-Za-z0-9_-]+$/.test(value))
		throw new Error(`Invalid integration session ID: ${value}`);
	return value;
};

const indexPathFor = async (
	git: GitProcess,
	worktreePath: string,
): Promise<string> => {
	const value = (
...
const markerRanges = async (
	path: string,
): Promise<GitConflictFile["markerRanges"]> => {
	try {
...
export const createIntegrationGitAdapter = ({
	...
	const sessionDirectory = (sessionId: string): string => {
		...
		if (!isInsideRoot(path))
			throw new Error("Integration path escapes its root.");
		return path;
	};
	...
	return {
		...
		async captureFingerprint(worktreePath) {
			const [{ stdout: head }, { stdout: status }, indexPath] =
				await Promise.all([
		...
		async createSyntheticSnapshot({
			...
			const statusFingerprintBefore =
				await this.captureFingerprint(worktreePath);
			const originalHeadSha = (
				await git.run({ cwd: worktreePath, args: ["rev-parse", "HEAD"] })
			).stdout.trim();
			const mergeBaseSha = (
				await git.run({
					cwd: repositoryPath,
					args: ["merge-base", targetCommitSha, originalHeadSha],
				})
			).stdout.trim();
			...
			try {
				await git.run({
					cwd: worktreePath,
					args: ["read-tree", originalHeadSha],
					env,
				});
				await git.run({
					cwd: worktreePath,
				...
				const tree = (
					await git.run({ cwd: worktreePath, args: ["write-tree"], env })
				).stdout.trim();
				const syntheticCommitSha = (
					await git.run({
						cwd: repositoryPath,
						args: [
							"commit-tree",
							...
							originalHeadSha,
							"-m",
							`Agentic Worktrees integration snapshot ${sessionId}/${side}`,
						],
						env: {
				...
				await git.run({
					cwd: repositoryPath,
					args: ["update-ref", syntheticRef, syntheticCommitSha],
				});
				const statusFingerprintAfter =
					await this.captureFingerprint(worktreePath);
				if (statusFingerprintAfter !== statusFingerprintBefore) {
					throw new Error(
						`Original worktree changed while capturing ${side} snapshot.`,
					);
				}
				return {
					originalHeadSha,
		...
		async createIntegrationWorktree({
			...
			await git.run({
				cwd: repositoryPath,
				args: ["worktree", "add", "-b", branch, path, targetCommitSha],
			});
			return { path, branch };
		},
		async mergeSynthetic(integrationPath, syntheticRef) {
			...
			try {
				await git.run({
					cwd: integrationPath,
					args: ["merge", "--no-ff", "--no-edit", syntheticRef],
				});
				const mergeCommitSha = (
					await git.run({ cwd: integrationPath, args: ["rev-parse", "HEAD"] })
				).stdout.trim();
		...
		async inspectConflicts(integrationPath) {
			const output = (
				await git.run({ cwd: integrationPath, args: ["ls-files", "-u", "-z"] })
			).stdout;
			const parsed = parseUnmerged(output);
			return Promise.all(
				[...parsed.entries()]
					.sort(([left], [right]) => left.localeCompare(right))
					.map(async ([path, stages]) => ({
						path,
		...
		async removeIntegrationWorktree({ repositoryPath, path, branch }) {
			if (!isInsideRoot(path))
				throw new Error("Integration cleanup path escapes its root.");
			if (existsSync(path))
				await git.run({
					cwd: repositoryPath,
		...
		async deleteSyntheticRef(repositoryPath, syntheticRef) {
			...
			try {
				await git.run({
					cwd: repositoryPath,
					args: ["update-ref", "-d", syntheticRef],
				});
			} catch (error) {
				if (!(error instanceof GitCommandError)) throw error;
			}
