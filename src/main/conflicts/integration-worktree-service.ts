...
export interface IntegrationPreparationInput {
	...
	targetBranch: string;
	participants: readonly [IntegrationParticipantInput, IntegrationParticipantInput];
	targets: OverlapTarget[];
	onStage?: (stage: "capturing" | "simulating" | "preparing_sandbox", label: string) => void;
}
...
export interface IntegrationWorktreeService {
	prepare(input: IntegrationPreparationInput): Promise<IntegrationPreparationResult>;
}
...
const mergeFiles = (
	targets: OverlapTarget[],
	gitFiles: Extract<Awaited<ReturnType<IntegrationGitAdapter["mergeSynthetic"]>>, { kind: "conflict" }>["files"],
): PreparedConflictFile[] => {
	const files = new Map(semanticFiles(targets).map((file) => [file.path, file]));
	...
	return [...files.values()].sort((left, right) =>
		(left.kind === right.kind ? left.path.localeCompare(right.path) : left.kind === "git_conflict" ? -1 : 1));
};
...
export const createIntegrationWorktreeService = ({
	...
	async prepare(input) {
		if (input.participants.some(({ repositoryId }) => repositoryId !== input.repository.id)) {
			throw new Error("Integration participants must belong to the same repository.");
		}
		if (input.participants[0].side !== "left" || input.participants[1].side !== "right") {
			throw new Error("Integration participants must be ordered left then right.");
		}

		const snapshots: Array<{ input: IntegrationParticipantInput; value: SyntheticSnapshotResult }> = [];
		let integration: IntegrationWorktreeResult | null = null;
		try {
			const targetCommitSha = await git.resolveRef(input.repository.path, input.targetBranch);
			...
			for (const item of input.participants) {
				...
				if (value.statusFingerprintBefore !== value.statusFingerprintAfter) {
					throw new Error(`Original ${item.side} worktree changed during capture.`);
				}
			...
			let gitResult = await git.mergeSynthetic(integration.path, snapshots[0]!.value.syntheticRef);
			if (gitResult.kind === "clean") {
				gitResult = await git.mergeSynthetic(integration.path, snapshots[1]!.value.syntheticRef);
			}
			const classification = classifyConfirmedConflict({ git: gitResult, targets: input.targets });
			const retained = classification !== "safe";
			const files = gitResult.kind === "conflict"
			...
			if (!retained) {
				...
				for (const snapshot of snapshots) {
					await git.deleteSyntheticRef(input.repository.path, snapshot.value.syntheticRef);
				}
				integration = null;
			} else {
			...
			return {
				targetCommitSha,
				classification,
				participants: snapshots.map(({ input: item, value }) => participant(item, value)),
				files,
			...
			if (integration) {
				cleanup.push(git.removeIntegrationWorktree({
					repositoryPath: input.repository.path,
					path: integration.path,
					branch: integration.branch,
				}));
			...
			for (const snapshot of snapshots) {
				cleanup.push(git.deleteSyntheticRef(input.repository.path, snapshot.value.syntheticRef));
			}
