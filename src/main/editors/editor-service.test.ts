...
describe("editor service", () => {
  it("lists only installed macOS applications and opens a worktree", async () => {
    ...
    const service = createEditorService({
      platform: "darwin",
      exists: (file) =>
        file === "/Applications/Cursor.app" || file === "/tmp/worktree",
      commandExists: vi.fn(),
      spawn,
    });
  ...
  it("rejects an editor ID outside the catalog", async () => {
    ...
    await expect(
      service.openEditor("unknown" as never, "/tmp/worktree"),
    ).rejects.toThrow("Unsupported editor: unknown");
  });
  ...
  it("discovers Linux editors through command resolution", async () => {
    ...
    const commandExists = vi.fn((command: string) =>
      Promise.resolve(command === "code"),
    );
    const service = createEditorService({
      platform: "linux",
  ...
  it("discovers Windows editors through command resolution", async () => {
    const commandExists = vi.fn((command: string) =>
      Promise.resolve(command === "cursor"),
    );
    const service = createEditorService({
      platform: "win32",
      exists: () => false,
