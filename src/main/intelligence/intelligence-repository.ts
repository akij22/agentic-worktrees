...
export interface IntelligenceRepository {
  replaceSnapshot(snapshot: PersistedIntelligenceSnapshot): PersistedIntelligenceSnapshot;
...
const parseJson = <Value>(value: string): Value => {
  try {
    ...
    throw new Error('Persisted intelligence JSON is invalid.', { cause: error });
  }
};
...
const asTargetType = (value: string): OverlapTargetType => {
  if (value === 'folder' || value === 'module' || value === 'file' || value === 'symbol') {
    return value;
  }
...
const insertSnapshotHierarchy = (
  ...
  database.insert(schema.intelligenceSnapshots).values({
    id: snapshot.id,
  ...
  for (const worktree of snapshot.worktrees) {
    ...
    database.insert(schema.intelligenceWorktrees).values({
      id: worktree.id,
    ...
    for (const file of worktree.files) {
      ...
      database.insert(schema.intelligenceChangedFiles).values({
        id: fileId,
      ...
      if (file.symbols.length > 0) {
        database.insert(schema.intelligenceChangedSymbols).values(
  ...
  snapshot.overlaps.forEach((overlap, overlapIndex) => {
    ...
    database.insert(schema.intelligenceOverlaps).values({
      id: overlap.id,
      snapshotId: snapshot.id,
    ...
    if (overlap.targets.length > 0) {
      database.insert(schema.intelligenceOverlapTargets).values(
        overlap.targets.map((item, targetIndex) => ({
          ...
          leftChangedFileId: item.leftFilePath
            ? fileIdByWorktreePath.get(`${overlap.leftWorktreeId}:${item.leftFilePath}`)
            : null,
          rightChangedFileId: item.rightFilePath
            ? fileIdByWorktreePath.get(`${overlap.rightWorktreeId}:${item.rightFilePath}`)
            : null,
...
const loadSnapshot = (
  ...
  const worktreeRows = database.select().from(schema.intelligenceWorktrees)
    .where(eq(schema.intelligenceWorktrees.snapshotId, snapshotRow.id)).all();
  const worktreeIds = worktreeRows.map(({ id }) => id);
  const fileRows = worktreeIds.length === 0 ? [] : database
    .select().from(schema.intelligenceChangedFiles)
    .where(inArray(schema.intelligenceChangedFiles.intelligenceWorktreeId, worktreeIds)).all();
  const fileIds = fileRows.map(({ id }) => id);
  const symbolRows = fileIds.length === 0 ? [] : database
    .select().from(schema.intelligenceChangedSymbols)
    .where(inArray(schema.intelligenceChangedSymbols.changedFileId, fileIds)).all();
  const overlapRows = database.select().from(schema.intelligenceOverlaps)
    .where(eq(schema.intelligenceOverlaps.snapshotId, snapshotRow.id)).all()
    .sort((left, right) => left.sortOrder - right.sortOrder);
  const overlapIds = overlapRows.map(({ id }) => id);
  const targetRows = overlapIds.length === 0 ? [] : database
    .select().from(schema.intelligenceOverlapTargets)
    .where(inArray(schema.intelligenceOverlapTargets.overlapId, overlapIds)).all();
  ...
  const worktrees = worktreeRows.map((row): PersistedIntelligenceWorktree => ({
    id: row.id,
    ...
    agentKind: row.agentKind === 'codex' || row.agentKind === 'opencode'
      ? row.agentKind
      : null,
    ...
    files: (filesByAnalysis.get(row.id) ?? [])
  ...
  for (const row of targetRows.sort((left, right) => left.sortOrder - right.sortOrder)) {
  ...
  const overlaps = overlapRows.map((row): ClassifiedOverlap & { id: string } => {
    const leftWorktreeId = worktreeIdByAnalysis.get(row.leftIntelligenceWorktreeId);
    const rightWorktreeId = worktreeIdByAnalysis.get(row.rightIntelligenceWorktreeId);
...
export const createIntelligenceRepository = (
  ...
  replaceSnapshot(snapshot) {
    database.transaction((transaction) => {
      transaction.delete(schema.intelligenceSnapshots)
        .where(eq(schema.intelligenceSnapshots.repositoryId, snapshot.repositoryId)).run();
  ...
  getLatestSnapshot(repositoryId) {
    const row = database.select().from(schema.intelligenceSnapshots)
      .where(eq(schema.intelligenceSnapshots.repositoryId, repositoryId)).get();
    return row ? loadSnapshot(database, row) : null;
  },

  getOverlap(overlapId) {
    const overlapRow = database.select().from(schema.intelligenceOverlaps)
      .where(eq(schema.intelligenceOverlaps.id, overlapId)).get();
    if (!overlapRow) throw new Error(`Intelligence overlap not found: ${overlapId}`);
    const snapshotRow = database.select().from(schema.intelligenceSnapshots)
      .where(eq(schema.intelligenceSnapshots.id, overlapRow.snapshotId)).get();
    if (!snapshotRow) throw new Error(`Intelligence snapshot not found: ${overlapRow.snapshotId}`);
    const snapshot = loadSnapshot(database, snapshotRow);
    const overlap = snapshot.overlaps.find(({ id }) => id === overlapId);
    if (!overlap) throw new Error(`Intelligence overlap not found: ${overlapId}`);
    const left = snapshot.worktrees.find(({ worktreeId }) =>
      worktreeId === overlap.leftWorktreeId);
  ...
  compareDiffs(overlapId, targetId) {
    ...
    if (targetId) {
      const row = database.select().from(schema.intelligenceOverlapTargets)
        .where(and(
          eq(schema.intelligenceOverlapTargets.id, targetId),
          eq(schema.intelligenceOverlapTargets.overlapId, overlapId),
        )).get();
      if (!row) throw new Error(`Intelligence overlap target not found: ${targetId}`);
      const leftPath = row.leftChangedFileId
        ? database.select({ path: schema.intelligenceChangedFiles.path })
            .from(schema.intelligenceChangedFiles)
            .where(eq(schema.intelligenceChangedFiles.id, row.leftChangedFileId)).get()?.path
        : undefined;
      const rightPath = row.rightChangedFileId
        ? database.select({ path: schema.intelligenceChangedFiles.path })
            .from(schema.intelligenceChangedFiles)
            .where(eq(schema.intelligenceChangedFiles.id, row.rightChangedFileId)).get()?.path
        : undefined;
      leftFiles = leftPath ? leftFiles.filter(({ path }) => path === leftPath) : leftFiles;
      rightFiles = rightPath ? rightFiles.filter(({ path }) => path === rightPath) : rightFiles;
    }
    return {
      overlapId,
      left: {
