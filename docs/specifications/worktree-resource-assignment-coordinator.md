...
Status: proposed specification for #66  
Parent: #57  
...
All counters are monotonically increasing 64-bit integers represented across IPC as decimal strings.
...
- `runtimeGeneration`: identity of one owned provider process generation.
- `runtimeAttestation`: `(agentKind, worktreeId, runtimeGeneration, verifiedGeneration, providerVersion, effectiveStateDigest)`.
...
A Resource update prepares one new pinned version generation for every affected Worktree. The distribution coordinator acquires affected Worktree writers in stable worktree-ID order, waits for all to be idle, and applies as one higher-level operation. Any Worktree failure rolls every accepted Worktree back before releasing any gate. Unrecoverable rollback closes only divergent Worktrees but the update is globally failed.

Uninstall is permitted only after a generated desired-state mutation removes the Resource from every affected Worktree and all removals verify. Installation content remains until no desired, verified, rollback, evidence-retention, or in-flight reference exists.
...
5. cascade Assignment state/attestations transactionally;
6. preserve installation and allowed activity evidence elsewhere;
...
Errors expose stable codes, safe user messages, current revision/phase, and allowed actions. Internal logs retain sanitized causal chains without prompts, outputs, credentials, Skill bodies, private paths, provider session IDs, or authorization material.
