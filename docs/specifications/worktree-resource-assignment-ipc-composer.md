...
Status: proposed specification for #68  
Parent: #57  
...
Create `src/shared/assignments/schemas.ts` as the only definition site for Assignment request, response, projection, event, state, action, and error contracts. `src/shared/ipc/channels.ts` contains channel constants only. Main, preload, and renderer import inferred types from the shared schema module; they do not duplicate string unions or interfaces.
...
`message` is a stable safe user-facing sentence selected by the main process. It never contains raw provider errors, prompts, output, settings, Skill content, credentials, authorization material, private paths, process commands, or provider session identifiers. Internal causes are logged separately with existing redaction policy.

## Channels

Add exactly these channels:
...
Main broadcasts only after the database/outbox commit to every live `webContents`, not only the initiating window or current session. Delivery rules:

- lower revision than local: ignore;
- equal revision and same event ID: idempotent ignore;
- equal revision with a different projection/event ID: refetch and report sanitized consistency failure;
- higher revision: replace the entire local projection;
- revision gap: replacement is still safe because event contains a full projection; optionally refetch for defense in depth;
...
Use one renderer hook keyed by `(worktreeId, currentAgentKind)` to own snapshot loading, invoke state, revision handling, and subscription. Session views consume it; they do not each implement event reconciliation.
...
- `src/shared/assignments/schemas.ts` — all contracts and inferred types;
- `src/shared/ipc/channels.ts` — six exact channels;
- `src/main/ipc/resource-assignment-handlers.ts` — parsing/error mapping/delegation;
- `src/main/ipc/index.ts` — registration only;
- `src/preload.ts` and global API declaration — narrow parsed API;
- `src/renderer/features/resources/hooks/useWorktreeResourceAssignment.ts` — snapshot/event/mutation state;
...
- one broadcast after durable outbox commit to all windows.
