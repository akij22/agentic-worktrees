...
Status: proposed specification for #65 and #57. This document defines behavior and implementation boundaries; it does not implement them.
...
`resourceDigest` is the verified immutable descriptor/content digest, not a Skill body or Capability package body. All generation fields are invocation-time snapshots; current Assignment/catalog state cannot rewrite them.
...
source = HMAC(keyVersion,
  ...
  adapterContractVersion, worktreeId, runtimeGeneration, rawScopedIdentity)
correlation = HMAC(keyVersion,
  "aw-resource-evidence/correlation/v1", worktreeId, runtimeGeneration,
  receiptKind, rawReceipt)
request = HMAC(keyVersion,
  "aw-resource-evidence/request/v1", provider, providerVersion,
  adapterContractVersion, worktreeId, runtimeGeneration,
...
sessionRoute = HMAC(keyVersion,
  ...
  worktreeId, runtimeGeneration, rawProviderSessionId)
```
...
Provider abort and host cancellation are separate observations. A trusted internal active-invocation registry is keyed by `(runtimeGeneration, rawInvocationId)` and associates the invocation with adapter dispatch context.
...
Quarantine is owned by the Worktree Runtime lifecycle, scoped to `(worktreeId, provider, runtimeGeneration)`. It blocks session create/resume/turn admission for that generation while leaving the narrow stop/cancel/drain lane available.
...
Production adds `resource_activity`, `resource_activity_evidence`, `resource_activity_session_routes`, and `resource_evidence_coverage` tables behind a repository owned by a `ResourceActivityEvidenceService` in the main process.
...
- Evidence insert and canonical projection update occur in one database transaction.
...
- Assignment/catalog/runtime generations are required positive integers.
...
- use the valid legacy `run_id` foreign key as the migration-only exact attribution of the application request; set `coverage = legacy_unverified`, `useState = not_confirmed`, `attribution = exact`, `routingIntegrity = unknown`, and `mode` only if the legacy value is valid; this exception cannot satisfy provider attribution or Use;
...
- use an explicit sentinel digest/generation contract for legacy-unverified identity; it can never satisfy new E2 matching.
...
- Preload exposes only a narrow session activity read/subscription capability scoped to an existing run ID.
- Main validates run ownership/existence for every request.
- Renderer cannot submit evidence, receipts, provider identity, status transitions, or cancellation invocation IDs.
- Logs use activity ID, Resource ID, enum states, provider/version, and generation numbers only. No raw evidence payload or private path.
- User-facing errors are generic; detailed sanitized reason codes remain main-process logs.
